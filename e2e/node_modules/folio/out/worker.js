...
process.on('message', async (message) => {
    if (message.method === 'init') {
        ...
        debug_1.setDebugWorkerIndex(workerIndex);
        debug_1.debugLog(`init`, message.params);
        return;
    }
    if (message.method === 'stop') {
        debug_1.debugLog(`stopping...`);
        await gracefullyCloseAndExit();
        debug_1.debugLog(`stopped`);
        return;
    }
    if (message.method === 'run') {
        debug_1.debugLog(`run`, message.params);
        testRunner = new workerRunner_1.WorkerRunner(message.params.entry, message.params.config, workerIndex);
...
async function gracefullyCloseAndExit() {
    ...
    if (testRunner)
        testRunner.stop();
    try {
        await workerRunner_1.fixturePool.teardownScope('test');
        await workerRunner_1.fixturePool.teardownScope('worker');
    }
    catch (e) {
...
function sendMessageToParent(method, params = {}) {
    try {
        if (method !== 'ready')
            debug_1.debugLog(`send`, { method, params });
